...
\begin{abstract}
  ...
  Replicating the data of a task on processors has a cost
  which we model this cost in two different way. The first model sets a
  maximum number of times a task can be replicated. The second model
  associate a memory cost to each task and introduce a second
  objective which is to minimize the memory occupation. We propose
  various algorithms with theoretical guarantees and lower bounds on
  achievable result to study the tradeoff between the number of
  replication and the guarantee on the makespan.
...
simply the maximum number of replica for each task: we will propose a
lower bound when no replication is allowed and propose algorithms for
the three cases where no replication is allowed, the tasks are
replicated everywhere and the number of replica is bounded by an
integer $k$. We introduced this model in~\cite{Chaubey15-APDCM}.
...
processor. In the {\bf memory-aware model}, tather than bounding the available memory, we treat the
maximum memory occupation as an objective to minimize in itself. Memory
...
follows. Section~\ref{sec:related} presents some related works. The
problem and in particular the model for uncertainty is presented in
Section~\ref{sec:model}. The replication bound model is investigated
in Section~\ref{sec:rbm} where we provide lower bound and analyze
three algorithms. The memory-aware model is investigated in
Section~\ref{sec:mem} where we analyze two bi-objective algorithms. We
conclude the paper in Section~\ref{sec:ccl}.
...
\subsection{Strategy 1: No Replication}
\label{sec:rbm-norep}
...
\subsection{Strategy 2: Replicate Data Everywhere}
...
\subsection{Strategy 3: Replication in groups}
...
\subsection{Summarizing the Replication Bound Model}
\label{sec:rbm-sum}
...
\section{Memory Aware Replications}
\label{sec:mem}
   
In the memory aware model, the problem is to schedule a set $J$ of $n$
tasks on $m$ machines such that both makespan $C_{max}$ as well as
memory usage $Mem_{max}$ is optimized. Each task $j$ is associated
with a memory cost $s_j$. Processor $i$ which has the data of the
tasks in $E_i$, as determined by the phase 1 algorithm, has a memory
occupation of $Mem_i = \sum_{j \in E_i} s_j$. The second objective is
to minimize $Mem_{max} = \max_i Mem_i$.

The algorithms we propose are inspired from the $SBO_\triangle$
algorithm~\cite{SDM2008} which is bi-objective algorithm for
minimizing makespan and memory usage for independent tasks with
certain processing time. $SBO_\triangle$ combines the two schedules
each dedicated to a single objective. $SBO$ stands for Symmetric
Bi-Objective which recalls that when the processing times are
certain, the two objectives are intercheangable.

Let $\pi_1$ be an offline schedule which minimizes the predicted
makespan and which schedules each task on one processor. Let
$\tilde{C}^{\pi_1}_{max}$ be the predicted makespan when all the tasks
are scheduled according to $\pi_1$ and $\tilde{C}^{*}_{max}$ be the
optimal predicted makespan. Similarly, let $\pi_2$ be a memory-aware
schedule of each task to a single processor. Let $Mem^{\pi_2}_{max}$
be the memory consumption of the most occupied machine in $\pi_2$ and
$Mem^*_{max}$ be the optimal memory consumption. Note that since
optimizing the predicted makespan and the memory consumption are
equivalent to the classical $P|p_i|C_{max}$ problem which admits a
PTAS~\cite{Hoch87}, we assume that $\pi_1$ is a $\rho_1$ approximated
schedule for the predicted makespan and that $\pi_2$ is a $\rho_2$
approximated schedule for the memory consumption.

The strategy is to divide tasks into two sets $S_1$ and
$S_2$ such that set $S_1$ contains the processing time intensive tasks
and set $S_2$ contains the memory intensive tasks, and schedule them
differently and in such a way that it optimizes both the objectives.
     
We propose two algorithms $SABO_\triangle$ (Static Asymmetric
Bi-Objective) and $ABO_\triangle$ (Asymmetric Bi-Objective), which are
based on $SBO_\triangle$. 
 
\subsection{The $SABO_\triangle$ Algorithm}
          
$SABO_\triangle$ is static in nature and
restricts each task to be scheduled to only one machine. Similar to
...
\begin{algorithm}                    
  ...
  \begin{algorithmic} 
    ...
    \ForAll{$j\in J$}
    \If{$\frac{\tilde{p_j}}{\tilde{C}^{\pi_1}_{max}} \leq \triangle \frac{s_j}{Mem^{\pi_2}_{max}}$}
...
\begin{algorithm}
  ...
  \begin{algorithmic} 
    ...
    \EndFor
    \ForAll{$j\in J$}
    \If{$\frac{\tilde{p_j}}{\tilde{C}^{\pi_1}_{max}} \geq \triangle \frac{s_j}{Mem^{\pi_2}_{max}}$}
...
\begin{proof}           
  ...
  $\sum\limits_{j \in S_1}^{} {Mem^{\pi_2}_{max}}
  \frac{\tilde{p_j}}{\triangle \tilde{C}^{\pi_1}_{max}} $ as per
  condition for $\pi_1$ scheduling, using this we have
  \begin{equation}\nonumber
    Mem_{max}\leq \sum_{j \in S_1}^{} {Mem^{\pi_2}_{max}} \frac{\tilde{p_j}}{\triangle \tilde{C}^{\pi_1}_{max}}+{Mem^{\pi_2}_{max}}
  \end{equation}
  Since $ \sum\limits_{j \in S_1}\tilde{p}_j \leq m\tilde{C}^{\pi_1}_{max} $, we have 
  \begin{equation}\nonumber
