...
\begin{lemma}\label{No Restriction}
  \textbf{LPT-No Restriction} has a optimal makespan of at least $  {\frac{2}{\alpha^{2}}} p_l $ when there are more tasks than number of machines in the schedule with $l$ as last task.
\end{lemma}
\begin{proof}
 ...
 Since number of tasks is greater than number of machines,  atleast one machine gets two tasks $c$ and $d$ assigned before last task $l$ in the schedule, such that $\tilde p_c \geq \tilde p_l$ and $\tilde p_d \geq \tilde p_l$. $C_{max}^{*}$ must be greater than sum of these two tasks.
   \begin{equation}\nonumber
    C_{max}^{*}\geq p_c + p_d
  \end{equation}	
  ...
  \end{equation}
Also, $\tilde p_l \geq \frac{1}{\alpha} p_l$
  \begin{equation}\nonumber
