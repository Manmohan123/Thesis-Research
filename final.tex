...
\begin{lemma}\label{No Restriction}
  \textbf{LPT-No Restriction} has a optimal makespan of at least $  {\frac{2}{\alpha^{2}}} p_l $ when there are more tasks than number of machines.
\end{lemma}
\begin{proof}
 ...
 Since number of tasks is greater than number of machines,  atleast one machine gets two tasks $c$ and $d$. $C_{max}^{*}$ must be greater than sum of these tasks.
   \begin{equation}\nonumber
    C_{max}^{*}\geq p_c + p_d
  \end{equation}	
  ...
  \begin{equation}\nonumber 
    C_{max}^{*} \geq \frac{1}{\alpha}\tilde p_c +  \frac{1}{\alpha} \tilde p_d 
  \end{equation}

  Let $c$ is scheduled before $d$ using LPT on the estimated processing times. So, $\tilde p_c\geq \tilde p_d$ holds true for tasks $c$ and $d$.
  Using this, we have
  \begin{equation}\nonumber
    C_{max}^{*} \geq \frac{2}{\alpha}\tilde p_d \geq {\frac{2}{\alpha^{2}}} p_d 
  \end{equation}
\end{proof}
...
\begin{proof}
  ...
  As $l$ is last task in the schedule there would be at least a task assigned before it to the same machine.   Using Lemma~\ref{No Restriction}, we have 
  \begin{equation}\nonumber
