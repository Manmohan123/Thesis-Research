...
is certainly feasible in practice as many system have more memory
...
single machine puts all the decision in phase 1: for each task, only one machine can be scheduled in phase 2.
...
processor that minimizes the sum of the $\tilde{p_j}$ of the tasks
...
Of course, there is an inherent trade off between replicating data and
...
When $\alpha=1.1$, the {\bf LS-Group} provides little
...
replication that can be achieved by putting the data of a task on only
